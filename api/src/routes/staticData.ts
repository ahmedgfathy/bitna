 ...
 */
router.get('/listing-statuses', tenantIsolation, async (req, res) => {
  try {
    const statuses = await db.getListingStatuses(req.tenantId!);
...
router.post('/listing-statuses', tenantIsolation, requireOwnerOrManager, async (req, res) => {
  try {
    const { name } = req.body;
    const status = await db.createListingStatus(name, req.tenantId!);
